...
float map(vec3 p) {
    ...
    d = max(d, -sd_box(point, vec3(5.5, 5.5, 13.)));
    d = max(d, -sd_box(point, vec3(5.5, 13., 5.5)));
    d = max(d, -sd_box(point, vec3(13., 5.5, 5.5)));
    ...
    d = max(d, -sd_sphere(point - vec3(space_mult / 2., space_mult / 2., space_mult / 2.), space_mult / 2. * 0.8));
